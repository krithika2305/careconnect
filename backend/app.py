...
from tensorflow.keras.layers import (
    GlobalAveragePooling2D, Dense, Dropout, BatchNormalization, DepthwiseConv2D
)
...
MODEL_PATH     = os.path.join(os.path.dirname(__file__), "alzheimers_model.h5")
...
def build_model() -> Model:
    ...
    Fine-tunes the top layers of the backbone.
    """
    base = MobileNetV2(
        weights="imagenet",
        include_top=False,
...
def get_model() -> Model:
    ...
    if _model is None:
        if os.path.exists(MODEL_PATH):
            ...
            _model = load_model(MODEL_PATH, custom_objects={'DepthwiseConv2D': CustomDepthwiseConv2D}, compile=False)
        else:
            logger.warning(
...
def preprocess_image(file_bytes: bytes) -> np.ndarray:
    ...
    # MobileNetV2 pre-processing (scales to [-1, 1])
...
# Pre-warm model at import time (works with both `flask run` AND `python app.py`)
logger.info("Pre-warming model at startup...")
get_model()
